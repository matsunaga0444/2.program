...
###################################################################################################################
##パラメータの調整
N, V, t, mu, gu, n0, n1, n2, nscf =10, 1, 1 , 0, 1, 2, 1, 100, 2000  # 7.525 #9.21
...
kBTs = np.linspace(0.0001,0.06,n2)
...
a_1 = calculate_a_1(ans[0,0,:,0], ans_F[0])
...
for i_q in range(n0-1):
    plt.scatter(kBTs, coherence_length_GL_formulation(ans[(i_q+1),0,:,0], ans[0,0,:,0], qs[(i_q+1)]),5 ,marker= "o" ,label ='Niklas coherence length')
    plt.scatter(kBTs, c_l_puterbation_extended_GL(a_1, b, d[i_q], e[i_q]), 5,marker= "o" ,label = 'modified coherence length')
    #plt.scatter(kBTs, c_l_puterbation_GL(a_1, d[i_q]), 5,marker= "o" ,label = 'conventional coherence length')
plt.legend()
