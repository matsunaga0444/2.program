...
##パラメータの調整
# N, V, t, mu =100, 1, 1, 0
# n_q, n_kBT, n_delta =5, 1, 100 # 7.525 #9.21
# wide_ans_q = 0.08
# qs   = linspace(0.0,0.01,n_q)            #(np.pi/a)
# kBTs = linspace(0.001,0.1,n_kBT)      
# ans_q= linspace(-1*wide_ans_q,wide_ans_q,n_delta)  
...
# for i_kBT in range(n_kBT):
#     plt.scatter(ans_q, ans[i_kBT,:,:], 5, c=ones(n_delta)*kBTs[i_kBT],  cmap='viridis' ,vmin=kBTs[0], vmax=kBTs[-1])
# c= plt.colorbar()
# plt.savefig("figure/delta-free_energy" + "_N_" + str(N) + "_V_" + str(V) + "_mu_" + str(mu) + "_t_" + str(t) + "_q_" + str(qs[0]) + ".png")
# plt.clf() 
