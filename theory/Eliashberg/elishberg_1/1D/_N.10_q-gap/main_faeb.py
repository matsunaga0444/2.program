...
class parameters:
    def __init__(self):
        self.nk1, self.nk2, self.nk3 = 10, 1, 1 
        self.beta_min   = 4
        self.beta_max   = 10000
        self.nbeta  = 1
        self.mu     = 0.1
...
class fmc_anisotropic_eliashberg_BCS:
    ...
    def scf(self, g, b, p):
        ...
        for n in range(p.nscf):
            if linalg.norm(self.delta_temp-self.delta)/linalg.norm(self.delta) <= 1e-9: break
            if abs(self.delta_temp[len(self.delta_temp)//2,0]) <= 1e-6: break
    ...
    def debug_plot(self, g, b, p):
        ...
        # plt.clf()
        for j in range(p.nk):
            plt.scatter(abs(b.iw_F), (self.delta[:,j].real), s=1)
        plt.savefig("figure/abs(b.iw_F)_Re(self.delta).png")
        plt.clf()
        for j in range(p.nk):
            plt.scatter(abs(b.iw_F), (self.delta[:,j].imag), s=1)
        plt.savefig("figure/abs(b.iw_F)_Im(self.delta).png")
        plt.clf()
        # for j in range(p.nk):
        ...
        # plt.clf()
        for j in range(p.nk):
            plt.scatter(arange(b.nbasis_F), log(abs(self.fkl[:,j])), s=1)
        plt.savefig("figure/num(b.basis_F)_log(abs(self.fkl)).png")
        plt.clf()
        # for j in range(p.nk):
