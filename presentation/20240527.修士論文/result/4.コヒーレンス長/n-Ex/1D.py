...
ans = []
for i in range(n):
    gap = Gap_equation_SC_BCS_1D.scf_1D(n0, n1, n2, kBTs, ini_gap, nscf, Ns[i], t, qs, mu, gu, Bs, V)
    x    = coherence_length_1D.coherence_length_from_extended_GL_theory_1D(t, mu, gu, Bs[0], kBTs[0], V, Ns[i], dq, gap[0,0,0,0])
    ans.append(x)
ans = array(ans)
