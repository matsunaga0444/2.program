...
xi = []
for i in range(n_U):
    ans = Gap_equation_SC_BCS_2D.scf_2D_simple(kBTs[0], ini_gap, nscf, N, t, 0, mu, gu, 0, Us[i])
    print(ans[0])
    xi_Ex_GL    = kappa_2D.kappa_from_Ex_GL_2D(t, mu, gu, Bs[0], kBTs[0], Us[i], N, ans[0], dq)
    xi.append(xi_Ex_GL)
xi = array(xi)
