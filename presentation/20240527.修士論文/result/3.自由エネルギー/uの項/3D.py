...
kBTs = linspace(0.001,0.01,n2)

ans = []
for i in range(n2):
    dc   =   Gap_equation_SC_BCS_3D.scf_3D_simple(kBTs[i], ini_gap, nscf, N, t, qs[0], mu, gu, Bs[0], V)
    x    =   free_energy_SC_BCS_3D.dda_3D(t, mu, gu, Bs[0], 1/kBTs[i], V, N, dc[0], dd)
    ans.append(x)
ans = array(ans)


# def bpp_3D(t, mu, gu, B, beta, V, N, dc, dq):
#     return (1/2)*d3df_q_3D(t, mu, gu, B, beta, V, N, dc, dq)
