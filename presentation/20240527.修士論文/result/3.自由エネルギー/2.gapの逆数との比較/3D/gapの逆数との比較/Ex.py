...
##パラメータの調整
N, V, t, mu, gu, n0, n1, n2, nscf =30, 2, 1 , 0, 1, 1, 1, 100, 2000  # 7.525 #9.21
...
kBTs = linspace(0.001,0.11,n2)

ans = Gap_equation_SC_BCS_3D.scf_3D(n0, n1, n2, kBTs, ini_gap, nscf, N, t, qs, mu, gu, Bs, V)

xi = []
for i in range(n2):
    xi_Ex_GL    = coherence_length_3D.coherence_length_from_extended_GL_theory_3D(t, mu, gu, Bs[0], kBTs[i], V, N, dq, ans[0,0,i,0])
    xi.append(xi_Ex_GL)
xi = array(xi)

########################################################################################################################
#plot the figure of comparing free energy to extended GL
plt.scatter(kBTs, 5.56346304502773*ans[0,0,0,0]/ans[0,0,:,0], 5)
