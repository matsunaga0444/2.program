...
##パラメータの調整
N = 5000
V, t, mu, gu, n0, n1, n2, n3, nscf =1, 1 , 0, 1, 100, 1, 1, 1, 2000  # 7.525 #9.21
n_search, error, check_gap =100, 1e-10, 1e-6
qs     = np.linspace(0.0,0.03,n0)         #(np.pi/a)
Bs     = np.linspace(0.0,0.0,n1)          #np.linspace(0,0.08,n1)
kBTs   = np.linspace(0.001,1,n2)
deltas = np.linspace(0.006, 0.15, n3)
beta = 1/kBTs[0]

###################################################################################################################
## gap_eq をdef
def e_k_spin(k1, k2, k3, q, y, B): 
    return 2*t*(np.cos((2*k1+(q/2)*np.pi))) - mu + y * 1/2 * gu * B
...
f, extended_GL = [], []
for j in range(n0):
    f_temp = free_energy(qs[j],deltas[0]) - free_energy(qs[0],deltas[0])
    f.append(f_temp)
f = np.array(f)
...
for i in range(n3):
    plt.scatter(qs, f[:], 5)
    plt.savefig("figure/q.dependent.Fq-F0.with.half.times.period.png")
    plt.clf()

###################################
##output
file = open("output/q.dependent.Fq-F0.with.half.times.period", "w")
file.write("##delta---Fq-F0" + "\n")
for j in range (n0):
