...
kBTs = linspace(0.18,0.195,n2)

ans = Gap_equation_SC_BCS_1D.scf_1D(n0, n1, n2, kBTs, ini_gap, nscf, N, t, qs, mu, gu, Bs, V)

j_q = []
for i in range(n0):
    j = penetration_depth_SC_1D.abs_free_energy_to_current(t, mu, gu, Bs[0], 1/kBTs[0], V, N, ans[i,0,0,0], dq, qs[i])
    j_q.append(j)
j_q = array(j_q)
...
plt.savefig("abs_j_q_GL_Ex.png")
plt.clf()

###################################
##output
file = open("abs_j_q_GL_Ex", "w")
for i in range(n0):
    file.write(str(qs[i]) + " " + str(j_q[i]) + " "  + "\n")
file.close()
