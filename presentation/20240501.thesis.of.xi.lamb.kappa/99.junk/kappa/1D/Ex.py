...
kBTs = linspace(0.001,0.195,n2)


xi = []
for i in range(n2):
    ans         = Gap_equation_SC_BCS_1D.scf_1D_simple(kBTs[i], ini_gap, nscf, N, t, qs[0], mu, gu, Bs[0], V)
    xi_Ex_GL    = kappa_1D.kappa_1D(t, mu, gu, Bs[0], kBTs[i], V, N, ans[0], dq)
    xi.append(xi_Ex_GL)
xi = array(xi)
