...
##パラメータの調整
N, V, t, mu, gu, n0, n1, n2, nscf =3000, 4, 1 , 0.1, 1, 1, 1, 100, 2000  # 7.525 #9.21
...
kBTs = linspace(0.001,0.60,n2)

ans = Gap_equation_SC_BCS_1D.scf_1D(n0, n1, n2, kBTs, ini_gap, nscf, N, t, qs, mu, gu, Bs, V)

lamb = []
for i in range(n2):
    lamb_linear = penetration_depth_SC_1D.penetration_depth_linear_1D_test3(t, mu, 1/kBTs[i], N, ans[0,0,i,0])
...
plt.savefig("lamb_linear_test3.png")
plt.clf()

###################################
##output
file = open("lamb_linear_test3", "w")
for i in range(n2):
