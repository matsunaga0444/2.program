...
f, extended_GL = [], []
for j in range(n0-1):
    df_q   = (free_energy(qs[j],deltas[0]) - free_energy(qs[0],deltas[0]))/N
    ddf_q1 = ((free_energy(qs[j+1],deltas[0]))-(free_energy(qs[j],deltas[0])))/((qs[j+1]-qs[j])*N)
    #ddf_q2 = ((free_energy(qs[j+2],deltas[0]))-(free_energy(qs[j+1],deltas[0])))/((qs[j+1]-qs[j])*N)
    #dddf_q = ddf_q2 - ddf_q1
    f.append(ddf_q1)
f = np.array(f)
