...
##パラメータの調整
N = 500
V, t, mu, gu, n0, n1, n2, n3, nscf =2.0, 1 , 0, 1, 1, 1, 1, 100, 2000  # 7.525 #9.21
...
for i in range(n0):
    f_1, extended_GL_1 = [], []
    for j in range(n3-2):
        #dfj   = (free_energy(qs[i],deltas[j]) - free_energy(qs[i],deltas[j-1]))/(deltas[j]-deltas[j-1])
        dfj1  = (free_energy(qs[i],deltas[j+1]) - free_energy(qs[i],deltas[j]))/(deltas[j+1]-deltas[j])
        dfj2  = (free_energy(qs[i],deltas[j+2]) - free_energy(qs[i],deltas[j+1]))/(deltas[j+2]-deltas[j+1])
        d2f = (dfj2-dfj1)/(deltas[j+2]-deltas[j+1])
        f_1.append(d2f)
    f.append(f_1)
f = np.array(f)
...
for i in range(n0):
    plt.scatter(deltas[0:98], f[i,:], 5, label=r"$F''(\Delta,0)$")
    plt.legend()
...
for i in range(n0):
    ...
    for j in range (n3-2):
        file.write(str(deltas[j]) + " " + str(f[i,j]) + "\n")
    file.close()
