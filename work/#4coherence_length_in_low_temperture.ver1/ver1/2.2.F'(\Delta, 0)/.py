...
for i in range(n0):
    f_1, extended_GL_1 = [], []
    for j in range(n3-1):
        #df   = (free_energy(qs[i],deltas[j]) - free_energy(qs[i],deltas[j-1]))/(deltas[j]-deltas[j-1])
        df   = (free_energy(qs[i],deltas[j+1]) - free_energy(qs[i],deltas[j]))/(deltas[j+1]-deltas[j])
        f_1.append(df)
    f.append(f_1)
f = np.array(f)
...
for i in range(n0):
    plt.scatter(deltas[1:100], f[i,:], 5, label=r"$F''(\Delta,0)$")
    plt.legend()
