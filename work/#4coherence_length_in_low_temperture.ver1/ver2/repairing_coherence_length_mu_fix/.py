...
###################################################################################################################
##パラメータの調整
N, V, t, mu, gu, n0, n1, n2, nscf =150, 1, 1 , 0, 1, 2, 1, 1, 2000  # 7.525 #9.21
...
for i_q in range(n0-1):
    plt.scatter(kBTs, coherence_length_GL_formulation(ans_a[(i_q+1),0,:,0], ans_a[0,0,:,0], qs[(i_q+1)]),5 ,marker= "o" ,label ='Niklas coherence length')
    plt.scatter(kBTs, c_l_puterbation_extended_GL(a, b, d[i_q]*ones(1), e[i_q]*ones(1)), 5,marker= "o" ,label = 'modified coherence length')
