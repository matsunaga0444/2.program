...
class fmc_anisotropic_eliashberg_BCS:
    ...
    def set_initial(self, g, b, p):
        ...
        self.chi_0   = zeros(b.niw_F)[:,None] * zeros(p.nk)[None,:]
    ...
    def scf(self, g, b, p):
        ...
        for n in range(p.n_scf):
            ...
            self.zeta, self.chi, self.delta_zeta = self.zeta_temp, self.chi_temp, self.delta_zeta_temp
            self.set_gkfs(g, b, p)

            self.set_frt_zeta(g, b, p)
            self.y_rt_zeta       = -(p.g0ph**2 * g.drt * self.frt_zeta).reshape(b.ntau_B,p.nk1,p.nk2,p.nk3) / p.nk
            self.y_kt_zeta       = fft.ifftn(self.y_rt_zeta,axes=(1,2,3))
            self.y_kt_zeta       = self.y_kt_zeta.reshape(b.ntau_B, p.nk)
    ...
    def set_frt_zeta(self, g, b, p):
        _ = (1/self.gkf_p + self.chi) *(1/conj(self.gkf_m) + self.chi) + self.delta_zeta*conj(self.delta_zeta)
