...
class gfunctions:
    ...
    def set_dkf_c(self, b, p):
        self.dkf =  -0.005 * ones(len((b.iw_B)-1))
        self.dkf = self.dkf[:,None] * ones(p.nk)[None,:]
...
class fmc_anisotropic_eliashberg_BCS:
    ...
    def scf(self, g, b, p):
        ...
        for n in range(100):
            ...
            self.set_fkt(g, b, p)
            self.y_kf = sum(-(p.g0ph**2 * g.dkf * self.fkt[0,:])) / p.nk 
    ...
    def set_fkt(self, g, b, p):
        ...
        self.fkt = dot(b.ul_F_tau_F, self.fkl)
        self.f_kl =  b.T_F.fit(self.fkt)
        self.f_kf =  (self.f_kl.T @ b.uhat_F).T
    ...
    def debug_plot(self, g, b, p):
        for j in range(p.nk):
            plt.scatter(arange(b.nbasis_F), abs(self.f_kl[:,j]), s=1)
        plt.savefig("figure/num(b.nbasis_F)_abs(self.f_kl).png")
        plt.clf()
        for j in range(p.nk):
            plt.scatter(abs(b.iw_F), abs(self.f_kf[:,j]), s=1)
        plt.savefig("figure/abs(b.iw_F)_abs(self.f_kf).png")
        plt.clf()
        for j in range(p.nk):
            plt.scatter(abs(b.iw_F), abs(self.fkf[:,j]), abs(self.f_kf[:,j]))
        plt.savefig("figure/abs(b.iw_F)_compare(abs(self.fkf)&abs(self.f_kf)).png")
        plt.clf()
