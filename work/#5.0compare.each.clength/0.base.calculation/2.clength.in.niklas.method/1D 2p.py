...
def coherence_length_GL_formulation_square(gap_q, gap_0, q):
    return (gap_0**2 - gap_q**2) / (gap_0**2*q**2*pi**2)
...
for i_q in range(n0-1):
    plt.scatter(kBTs, coherence_length_GL_formulation(ans_a[(i_q+1),0,:,0], ans_a[0,0,:,0], qs[(i_q+1)]),5 ,marker= "o" ,label =r'$\xi$ in conventional method')
    #plt.scatter(kBTs, c_l_puterbation_extended_GL(a, b, d[i_q]*ones(1), e[i_q]*ones(1)), 5,marker= "o" ,label = r'$\xi$ in modified method')
