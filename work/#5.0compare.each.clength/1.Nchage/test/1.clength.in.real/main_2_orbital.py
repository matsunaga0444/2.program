...
class parameters:
    def __init__(self):
        ...
        self.beta = 1/0.002
        self.mu   = 0
        self.u    = 3.9 * ones(self.nk)
        self.u[3*self.nk//4:-1] = 0.0
        self.nscf = 1000
        self.t1 = 1
        self.t2 = 2
        self.ini_gap = 0.4
...
class calculation:
    def __init__(self, p):
        ...
        self.array = zeros((p.nk-1,))
        self.array[0::2] = p.t1
        self.array[1::2] = p.t2
        self.H_normal = - (diag(self.array,k=1) + diag(self.array,k=-1)) - p.mu * eye(p.nk)
        self.H_normal[0,-1] = self.H_normal[-1,0] = -1 * p.t2
        self.delta    = p.ini_gap*eye(p.nk)
        self.delta0   = p.ini_gap*eye(p.nk)
