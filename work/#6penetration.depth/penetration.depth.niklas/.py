...
###################################################################################################################
#for calculation niklas penetration depth
def v_k(k1, k2, k3):
    return 2 * t * (cos(k1*np.pi)+cos(k2*np.pi)+cos(k3*np.pi))

def G_f(k1, k2, k3, gap, q, B):
    a = (-1 * e_k_a + sqrt(E_k_q_s(k1, k2, k3, gap, q, -1, B)))/ 2 * E_k_q_s(k1, k2, k3, gap, q, -1, B)
    b = (     e_k_a + sqrt(E_k_q_s(k1, k2, k3, gap, q, -1, B)))/ 2 * E_k_q_s(k1, k2, k3, gap, q, -1, B)
    return -1 * (a*(1-Fermi(beta, E_k_q_s(k1, k2, k3, gap, q, -1, B))))+(b*Fermi(beta, E_k_q_s(k1, k2, k3, gap, q, 1, B)))

def current(k1, k2, k3, n_k, gap, q, B):
    j = v_k(k1, k2, k3) * G_f(k1, k2, k3, gap, q, B)
    return 2 * j/n_k
...
ky, kz = 1, 1
for i in range(n_q):
    j = current(kx, ky, kz, N, d_c, qs[i], Bs[0])
