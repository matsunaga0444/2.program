...
f, extended_GL = [], []
for j in range(n3):
    f_temp = (F1(qs[1],deltas[j]) - F1(qs[0],deltas[j]))/N
    f.append(f_temp)
f = np.array(f)
...
for i in range(n0):
    ...
    plt.savefig("figure/delta.dependent.f1q-f10.png")
    plt.clf()

###################################
##output
file = open("output/delta.dependent.f1q-f10", "w")
file.write("##delta---f1q-f10" + "\n")
for j in range (n3):
