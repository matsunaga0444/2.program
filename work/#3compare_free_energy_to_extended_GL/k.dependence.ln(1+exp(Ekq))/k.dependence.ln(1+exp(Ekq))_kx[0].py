...
for j in range(n3):
    print(E_k_q_s(kx[0][0], ky, kz, deltas[j], qs[1], y[0][0], Bs[0]))
    print(E_k_q_s(kx[0][0], ky, kz, deltas[j], qs[0], y[0][0], Bs[0]))
    print(exp(-1*beta*E_k_q_s(kx[0][0], ky, kz, deltas[j], qs[1], y[0][0], Bs[0])))
    print(exp(-1*beta*E_k_q_s(kx[0][0], ky, kz, deltas[j], qs[0], y[0][0], Bs[0])))
    f_temp = log(1+exp(-1*beta*E_k_q_s(kx[0][0], ky, kz, deltas[j], qs[1], y[0][0], Bs[0])))
    f.append(f_temp)
f = np.array(f)
...
for i in range(n0):
    ...
    plt.savefig("figure/k.dependence.ln(1+exp(Ekq))_kx[0].png")
    plt.clf()

###################################
##output
file = open("output/k.dependence.ln(1+exp(Ekq))_kx[0]", "w")
