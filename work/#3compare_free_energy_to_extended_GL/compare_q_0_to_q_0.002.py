...
f, extended_GL = [], []
for j in range(n3):
    f_temp = free_energy(qs[1],deltas[j]) - free_energy(qs[0],deltas[j])
    f.append(f_temp)
f = np.array(f)
...
for i in range(n0):
    ...
    plt.savefig("figure/compare_q_0_to_finite_q.png")
    plt.clf()

###################################
##output
file = open("output/compare_q_0_to_finite_q", "w")
file.write("##delta---compare_q_0_to_finite_q" + "\n")
for j in range (n3):
