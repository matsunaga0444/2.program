...
for j in range(n3):
    print(kx[1230][0])
    print(E_k_q_s(kx[1230][0], ky, kz, deltas[j], qs[1], y[0][0], Bs[0]))
    print(E_k_q_s(kx[1230][0], ky, kz, deltas[j], qs[0], y[0][0], Bs[0]))
    print(exp(-1*beta*E_k_q_s(kx[1230][0], ky, kz, deltas[j], qs[1], y[0][0], Bs[0])))
    print(exp(-1*beta*E_k_q_s(kx[1230][0], ky, kz, deltas[j], qs[0], y[0][0], Bs[0])))
    f_temp = log(1+exp(-1*beta*E_k_q_s(kx[1230][0], ky, kz, deltas[j], qs[1], y[0][0], Bs[0])))-log(1+exp(-1*beta*E_k_q_s(kx[1230][0], ky, kz, deltas[j], qs[0], y[0][0], Bs[0])))
    f.append(f_temp)
f = np.array(f)
...
for i in range(n0):
    ...
    plt.savefig("figure/kx[1230][0].png")
    plt.clf()

###################################
##output
file = open("output/kx[1230][0]", "w")
