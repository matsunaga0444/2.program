...
for j in range(n3):
    ...
    f_temp = E_k_q_s(kx[1255][0], ky, kz, deltas[j], qs[1], y[0][0], Bs[0])-E_k_q_s(kx[1255][0], ky, kz, deltas[j], qs[0], y[0][0], Bs[0])
    f.append(f_temp)
f = np.array(f)
...
for i in range(n0):
    ...
    plt.savefig("figure/k.dependence.Ekq-Ek0.png")
    plt.clf()

###################################
##output
file = open("output/k.dependence.Ekq-Ek0", "w")
