...
def inverse_xsi_k(qs):
    k1 = -1 * np.pi + 2 * arange(N) * np.pi / (N)
    kx = k1
    ky, kz = 1, 1
    f = 1/(abs(e_k_spin(-1*kx, -1*ky, -1*kz, qs,-1, Bs[0])))
    return sum(f)
...
for j in range(n3):
    print(inverse_xsi_k(qs[0]))
    f_temp = (inverse_xsi_k(qs[0]) * qs[1]**2 *pi**2) / (16*N)
    f.append(f_temp)
f = np.array(f)
...
for i in range(n0):
    ...
    plt.savefig("figure/k.dependence.inverse_xsi_k.png")
    plt.clf()

###################################
##output
file = open("output/k.dependence.inverse_xsi_k", "w")
