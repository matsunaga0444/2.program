...
for j in range(n3):
    ...
    f_temp = log(1+exp(-1*beta*E_k_q_s(kx[1255][0], ky, kz, deltas[j], qs[0], y[0][0], Bs[0])))
    f.append(f_temp)
f = np.array(f)
...
for i in range(n0):
    ...
    plt.savefig("figure/k.dependence.ln(1+exp(Ek0)).png")
    plt.clf()

###################################
##output
file = open("output/k.dependence.ln(1+exp(Ek0))", "w")
file.write("##delta---ln(1+exp(Ek0))" + "\n")
for j in range (n3):
