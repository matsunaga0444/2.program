...
for i in range(n2):
    ...
    ###################################################################################################################
    ##\xi
    xi.append(sqrt(dddf_q/(dc**2 * d2f)))
xi = np.array(xi)

########################################################################################################################
# #plot the figure of comparing free energy to extended GL
# figure= plt.scatter(kBTs, xi, 5, c="red", marker=".",zorder=2) #, label= 'coherence length'
# print(kBTs)
# figure= plt.plot(kBTs, 2.11173/tanh(1.65984*sqrt(0.1962541806020067/kBTs-1)), 5, zorder=1) #label='extrapolate plot',
# figure=plt.tick_params(labelsize=18)
# figure=plt.legend(fontsize=16)
# figure=plt.savefig("figure/q-DeltaF.png")
# figure=plt.clf

#plot the figure of comparing free energy to extended GL
figure=plt.plot(kBTs, 2.11173/tanh(1.65984*sqrt(0.1962541806020067/kBTs-1)), 10, c="red",label=r'fitting plot $\xi(T) = \xi_0 \frac{\Delta_0}{\Delta(T)}$', zorder=1) #
figure=plt.scatter(kBTs, xi, 10, marker=".", label= 'coherence length',zorder=2) #
figure=plt.tick_params(labelsize=18)
figure=plt.legend(fontsize=16)
figure=plt.savefig("figure/q-DeltaF.png")
figure=plt.clf
