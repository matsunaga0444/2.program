...
from SC_coherence_length import coherence_length_1D_free
...
def G_f_1D(t, k1, gap, q, mu, gu, B, beta):
    a = ( -1 * Gap_equation_SC_BCS_1D_free.e_k_s_1D(t, k1, q, mu, gu, B) \
         + Gap_equation_SC_BCS_1D_free.E_k_q_1D(t, k1, q, mu, gu, B, gap))/ \
            (2 * Gap_equation_SC_BCS_1D_free.E_k_q_1D(t, k1, q, mu, gu, B, gap))
    b = (Gap_equation_SC_BCS_1D_free.e_k_s_1D(t, k1, q, mu, gu, B) \
         + Gap_equation_SC_BCS_1D_free.E_k_q_1D(t, k1, q, mu, gu, B, gap))/ \
            (2 * Gap_equation_SC_BCS_1D_free.E_k_q_1D(t, k1, q, mu, gu, B, gap))
    return (a*(1-fermi.Fermi(beta, Gap_equation_SC_BCS_1D_free.E_k_q_s_1D(t, k1, q, mu, -1, gu, B, gap)))) \
           +(b*fermi.Fermi(beta, Gap_equation_SC_BCS_1D_free.E_k_q_s_1D(t, k1, q, mu, 1, gu, B, gap)))
...
def max_current(nscf, N, t, n_k, xi, mu, gu, B, beta, V):
    max_q = 1/(sqrt(3)*(xi))
    dc_q  = Gap_equation_SC_BCS_1D_free.scf_1D_simple(1/beta, 100, nscf, N, t, max_q/pi, mu, gu, B, V)
...
#             * coherence_length_1D_free.coherence_length_from_GL_theory_1D(dq, kBT, ini_gap, nscf, N, t, mu, gu, B, V) \
...
def penetration_depth_from_extended_GL_1D(t, mu, gu, B, beta, V, N, dc, dq):
    return sqrt(1/(32*pi*free_energy_SC_BCS_1D_free.bpp_1D(t, mu, gu, B, beta, V, N, dc, dq)))

def free_energy_to_current(t, mu, gu, B, beta, V, N, dc, dq, q, dc_q):
    return 4*(free_energy_SC_BCS_1D_free.bpp_1D(t, mu, gu, B, beta, V, N, dc, dq)/dc**2) * dc_q **2 * q * pi

def free_energy_to_max_current(nscf, xi, t, mu, gu, B, beta, V, N, dq):
    max_q = 1/(sqrt(3)*(xi))
    dc_q  = Gap_equation_SC_BCS_1D_free.scf_1D_simple(1/beta, 100, nscf, N, t, max_q/pi, mu, gu, B, V)
    return 4*free_energy_SC_BCS_1D_free.bpp_1D(t, mu, gu, B, beta, V, N, dc_q[0], dq) * max_q/pi
...
def penetration_depth_linear_1D(t, mu, beta, N, gap):
    ...
    e_k = electron_band_free.e_k_s_1D(t, k1, 0, mu, 0, 0, 0)
